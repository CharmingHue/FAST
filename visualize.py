...
def visual(get_ann, data_dir, gt_dir, pred_dir, dataset, config):
    ...
    for index, (img_path, gt_path, pred_path) in tqdm(enumerate(zip(img_paths, gt_paths, pred_paths)), total=len(img_paths)):
        ...
        if args.show_gt:
            ...
            mmcv.mkdir_or_exist(f"visual/{dataset}/{config}")
            img.save(f"visual/{dataset}/{config}/{index}.png") # save images into visual/
        else:
            img_pred = draw(img, pred, _) # draw predictions on images
            img = Image.fromarray(img)
            mmcv.mkdir_or_exist(f"visual/{dataset}/{config}")
...
def visual_gt(get_ann, data_dir, gt_dir, dataset, config):
    img_names = [img_name for img_name in mmcv.utils.scandir(data_dir, '.jpg')]
    img_names.extend([img_name for img_name in mmcv.utils.scandir(data_dir, '.png')])
    img_names.extend([img_name for img_name in mmcv.utils.scandir(data_dir, '.JPG')])
    ...
    for index, (img_path, gt_path) in tqdm(enumerate(zip(img_paths, gt_paths)), total=len(img_paths)):
        img = get_img(img_path) # load image
        gt, word = get_ann(img, gt_path) # load annotation
        ...
        mmcv.mkdir_or_exist(f"visual/{dataset}/{config}")
        img_gt.save(f"visual/{dataset}/{config}/{index}.png") # save images into visual/
        img = Image.fromarray(img)
        mmcv.mkdir_or_exist(f"visual/{dataset}/ori")
        img.save(f"visual/{dataset}/ori/{index}.png") # save images into visual/
...
if __name__ == '__main__':
    ...
    msra_pred_dir = 'outputs/{}/submit_msra/'.format(args.config)
    ctw_pred_dir = 'outputs/{}/submit_ctw/'.format(args.config)
    tt_pred_dir = 'outputs/{}/submit_tt/'.format(args.config)
    ic15_pred_dir = 'outputs/{}/submit_ic15/'.format(args.config)
    ...
    print(test_data_dir)
    if args.config =='gt':
        visual_gt(get_ann, test_data_dir, test_gt_dir, args.dataset, args.config)
    else:
        visual(get_ann, test_data_dir, test_gt_dir, pred_dir, args.dataset, args.config)